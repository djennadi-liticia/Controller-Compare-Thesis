...
public class SDNWiseMessage {
    ...
    public static NetworkPacket setPayload(NetworkPacket np, byte[] payload) {
        ...
        //np.setArray(packet);
        np.setPayload(packet);
        return np;
    }

    public static byte[] getPayload(NetworkPacket np) {
        byte[] packet = np.toByteArray();
        return Arrays.copyOfRange(packet,SRC_INDEX, np.getLen());
    }
...
//        int networkPacketType = networkPacket.getType();
        ...
        if (sensorMessageType == OPEN_PATH) {
            ...
            ReportPacket reportPacket = new ReportPacket(getPayload(networkPacket));
            sdnWiseMessage = new SDNWiseReportMessage(reportPacket.getDistance(), reportPacket.getBattery(),
                    reportPacket.getLinkQuality(1), ipAddress, port);
...
//        int networkPacketType = networkPacket.getType();
        ...
        switch (sensorMessageType) {
            case DATA:
                ...
                break;
            case BEACON:
                BeaconPacket beaconPacket = new BeaconPacket(getPayload(networkPacket));
