...
import static org.onosproject.usdn.protocol.USDNBuiltinMessageType.CONFIG;

public class USDNFunctionLoadMessage extends USDNMessage {
    ...
    private static final int FUNCTION_HEADER_LEN = 3, CONFIG_HEADER_LEN = 1,
            FUNCTION_PAYLOAD_LEN = USDNnetworkPacket.MAX_PACKET_LENGTH - (USDNnetworkPacket.DFLT_HDR_LEN
    ...
    public USDNFunctionLoadMessage(SensorNodeAddress sink, SensorNodeAddress dest,
                                      byte functionId, URI classLocation) {
        super.setMessageType(CONFIG);
        super.setSource(new USDNnodeId(sink.getNetId(), sink.getAddr()));
        super.setDestination(new USDNnodeId(dest.getNetId(), dest.getAddr()));
    ...
    public USDNFunctionLoadMessage(SensorNodeAddress sink, SensorNodeAddress dest, byte[] classBuffer) {
    ...
    public List<USDNnetworkPacket> getNetworkPackets() {
        ...
        LinkedList<USDNnetworkPacket> ll = new LinkedList<>();
